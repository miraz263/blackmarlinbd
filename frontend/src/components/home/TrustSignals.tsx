...
const CERTIFICATIONS: Certification[] = [
  { name: "ISO 27001",      descKey: "trust.cert_info_security",     status: "In Progress" },
  { name: "ISO 9001",       descKey: "trust.cert_quality_mgmt",      status: "Planned" },
  { name: "PCI DSS Level 1",descKey: "trust.cert_payment_security",  status: "Planned" },
  { name: "SOC 2 Type II",  descKey: "trust.cert_service_security",  status: "Planned" },
  { name: "GDPR Compliant", descKey: "trust.cert_data_privacy",      status: "Active" },
  { name: "HIPAA Ready",    descKey: "trust.cert_healthcare_privacy", status: "Active" },
];
...
type StatusConfig = { labelKey: string; classes: string; icon: typeof CheckCircle2; dotColor: string };

const STATUS_CONFIG: Record<CertStatus, StatusConfig> = {
  Active: {
    labelKey: "trust.status_active",
    classes: "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20",
    icon: CheckCircle2,
    dotColor: "bg-emerald-400",
  },
  "In Progress": {
    labelKey: "trust.status_in_progress",
    classes: "bg-amber-500/10 text-amber-400 border border-amber-500/20",
    icon: Clock,
    dotColor: "bg-amber-400",
  },
  Planned: {
    labelKey: "trust.status_planned",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{t(cert.descKey)}</p>
        </div>
        <span className={`shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${cfg.classes}`}>
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <h2 className="text-4xl sm:text-5xl font-bold text-foreground mb-4">
            <span className="gradient-text">{t("trust.heading")}</span>
          </h2>
        ...
        <div className="mb-16">
          ...
          >
            {(Object.entries(STATUS_CONFIG) as [CertStatus, StatusConfig][]).map(([key, cfg]) => (
